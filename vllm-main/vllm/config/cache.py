...
CacheDType = Literal[
    ...
    "fp8_inc",
    "fp8_ds_mla",
    # ECC-protected INT4 cache types
    "int4_ecc",           # INT4 + Hamming(8,4) SECDED
    "int4_h74",           # INT4 + Hamming(7,4) SEC
    "int4_golay",         # INT4 + Golay(24,12) TEC
    "int4_golay_hybrid",  # Golay triplets + Hamming remainder
    "int4_rs",            # INT4 + Reed-Solomon(12,8) - corrects 2 symbols
]
...
class CacheConfig:
    ...
    def _validate_cache_dtype(cls, cache_dtype: CacheDType) -> CacheDType:
        ...
        elif cache_dtype.startswith("int4"):
            codec_info = {
                "int4_ecc": "Hamming(8,4) SECDED (single-error correct, double-error detect)",
                "int4_h74": "Hamming(7,4) SEC (single-error correct)",
                "int4_golay": "Golay(24,12) TEC (triple-error correct)",
                "int4_golay_hybrid": "Hybrid Golay+Hamming (triplets: Golay, remainder: Hamming)",
            }
            codec_desc = codec_info.get(cache_dtype, "unknown ECC")
            logger.info(
                "Using INT4 quantization with ECC protection for KV cache. "
                "ECC codec: %s. This reduces memory footprint while providing "
