...
def reshape_and_cache_golay_hybrid(
    key: torch.Tensor,
    value: torch.Tensor,
    key_cache: torch.Tensor,
    value_cache: torch.Tensor,
    ...
    """
    Reshape and cache with Golay(24,12) + Hamming(8,4) hybrid ECC encoding.

    Encodes INT4 triplets with Golay for up to 3-bit error correction,
    and remainder elements with Hamming(8,4) for SECDED protection.

    Memory layout per head: [golay_triplets (4B each)] [hamming_remainder (1B each)]
    """
    torch.ops._C_cache_ops.reshape_and_cache_golay_hybrid(
        key,
        value,
        key_cache,
        value_cache,
...
def inject_cache_errors(
    ...
    """
    Inject random bit errors into ECC-encoded KV cache for fault tolerance testing.

    This function flips random bits in the cache with the specified probability,
    simulating hardware bit errors for evaluating ECC protection effectiveness.

    Args:
        key_cache: ECC-encoded key cache tensor [num_blocks, ...]
        value_cache: ECC-encoded value cache tensor [num_blocks, ...]
        bit_error_rate: Probability of flipping each bit (e.g., 1e-6)
        seed: Random seed for reproducibility
    """
    torch.ops._C_cache_ops.inject_cache_errors(
        key_cache, value_cache, bit_error_rate, seed
    )
...
def cp_gather_and_ecc_decode_kv_cache(
    ...
    Used by FlashAttention backends that don't have native ECC support.
    Gathers paged cache blocks, decodes ECC codewords, and writes decoded
    values to contiguous workspace tensors.

    Args:
        src_cache_k: ECC-encoded key cache [NUM_BLOCKS, BLOCK_SIZE, NUM_HEADS, HEAD_SIZE]
        src_cache_v: ECC-encoded value cache [NUM_BLOCKS, BLOCK_SIZE, NUM_HEADS, HEAD_SIZE]
        dst_k: Decoded key workspace [TOT_TOKENS, NUM_HEADS, HEAD_SIZE]
        dst_v: Decoded value workspace [TOT_TOKENS, NUM_HEADS, HEAD_SIZE]
        block_table: Block indices [BATCH, BLOCK_INDICES]
