...
class LLM:
    ...
    def get_kv_cache(self) -> list:
        """Get KV cache tensors for fault injection and evaluation.

        Returns:
            List of KV cache tensors, one per layer.
        """
        return self.llm_engine.get_kv_cache_tensors()

    def inject_cache_errors(self, ber: float, seed: int) -> None:
        """Inject bit errors into KV cache at specified BER.

        This method injects random bit errors into the ECC-encoded KV cache
        for fault tolerance testing. Only applicable when using ECC cache modes
        (int4_ecc, int4_h74, int4_golay, int4_golay_hybrid).
        ...
        Raises:
            ValueError: If cache dtype is not an ECC type.
            RuntimeError: If KV cache format is unexpected.
        """
        import torch

        # Validate cache dtype is ECC-protected
        cache_dtype = self.llm_engine.cache_config.cache_dtype
        ecc_types = ("int4_ecc", "int4_h74", "int4_golay", "int4_golay_hybrid")
        if cache_dtype not in ecc_types:
            raise ValueError(
                f"inject_cache_errors only works with ECC cache types "
                f"({', '.join(ecc_types)}), but got cache_dtype='{cache_dtype}'"
            )

        kv_caches = self.get_kv_cache()
        for layer_idx, kv_cache in enumerate(kv_caches):
            # Use per-layer seed for reproducibility
            layer_seed = seed + layer_idx * 1000

            # KV cache format is (2, num_blocks, block_size, num_kv_heads, head_size)
            # where cache[0] is key and cache[1] is value
            if kv_cache.dim() < 1 or kv_cache.shape[0] != 2:
                raise RuntimeError(
                    f"Expected KV cache with shape (2, ...) but got {kv_cache.shape}. "
                    f"Layer {layer_idx} may use an unsupported cache format."
                )

            key_cache = kv_cache[0].contiguous()
            value_cache = kv_cache[1].contiguous()

            # Validate dtype is ECC-encoded (uint8 for Hamming/hybrid, int32 for pure Golay)
            if key_cache.dtype not in (torch.uint8, torch.int32):
                raise RuntimeError(
                    f"Expected uint8 or int32 ECC-encoded cache but got {key_cache.dtype}. "
                    f"Cache dtype '{cache_dtype}' may not be properly configured."
                )

            # Call CUDA fault injection kernel
            torch.ops._C_cache_ops.inject_cache_errors(
                key_cache, value_cache, ber, layer_seed
            )
