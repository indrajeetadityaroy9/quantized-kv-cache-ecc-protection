...
TORCH_LIBRARY_EXPAND(CONCAT(TORCH_EXTENSION_NAME, _cache_ops), cache_ops) {
  ...
  // Gather and ECC-decode INT4 KV cache to FP16 workspace
  // Used by FlashAttention backends that don't have native ECC support
  cache_ops.def(
      "cp_gather_and_ecc_decode_kv_cache("
      "Tensor src_cache_k, Tensor src_cache_v, "
      "Tensor! dst_k, Tensor! dst_v, "
  ...
  // Reshape and cache with Golay(24,12) + Hamming(8,4) hybrid encoding
  cache_ops.def(
      "reshape_and_cache_golay_hybrid(Tensor key, Tensor value,"
      "                               Tensor! key_cache, Tensor! value_cache,"
      "                               Tensor slot_mapping,"
      "                               Tensor k_scale, Tensor v_scale) -> ()");
  cache_ops.impl("reshape_and_cache_golay_hybrid", torch::kCUDA,
                 &reshape_and_cache_golay_hybrid);

  // ECC fault injection for testing INT4-ECC protected KV cache
  cache_ops.def(
      "inject_cache_errors(Tensor! key_cache, Tensor! value_cache, "
      "float bit_error_rate, int seed) -> ()");
  cache_ops.impl("inject_cache_errors", torch::kCUDA, &inject_cache_errors);
}
