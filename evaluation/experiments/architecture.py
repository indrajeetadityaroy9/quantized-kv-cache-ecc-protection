...
def analyze_architecture(model: Any, model_name: str) -> ArchitectureInfo:
    ...
    if layers:
        ...
        projection_style = (
            "combined_qkv" if "c_attn" in first_layer_name else "separate_kv"
        )
    else:
        layer_type = "Unknown"
...
def run_architecture_comparison(
    ...
    llama_info = analyze_architecture(
        llama_model, getattr(llama_model.config, "_name_or_path", "llama")
    )
    ...
    if verbose:
        ...
        print("=" * 60)
        print(
            f"GPT-2: {gpt2_info.layer_type}, {gpt2_info.n_kv_projections} projections"
        )
        print(
            f"LLaMA: {llama_info.layer_type}, {llama_info.n_kv_projections} projections"
        )
        print()
    ...
    with torch.no_grad():
        gpt2_clean_logits = generate_clean_logits(
            gpt2_model, gpt2_tokenizer, texts, max_length=128, device="cuda"
        )
...
def generate_comparison_report(result: ComparisonResult) -> str:
    ...
    lines.append("-" * 80)
    lines.append(
        f"{'Model Name':<25} {gpt2_info.model_name:<25} {llama_info.model_name:<25}"
    )
    lines.append(
        f"{'Layer Type':<25} {gpt2_info.layer_type:<25} {llama_info.layer_type:<25}"
    )
    lines.append(
        f"{'Projection Style':<25} {gpt2_info.projection_style:<25} {llama_info.projection_style:<25}"
    )
    lines.append(
        f"{'Num Layers':<25} {gpt2_info.n_layers:<25} {llama_info.n_layers:<25}"
    )
    lines.append(
        f"{'KV Projections':<25} {gpt2_info.n_kv_projections:<25} {llama_info.n_kv_projections:<25}"
    )
    lines.append(
        f"{'Hidden Size':<25} {gpt2_info.hidden_size:<25} {llama_info.hidden_size:<25}"
    )
    lines.append(
        f"{'Attention Heads':<25} {gpt2_info.n_heads:<25} {llama_info.n_heads:<25}"
    )
    lines.append(
        f"{'Total Parameters':<25} {gpt2_info.total_params:,d}{'':<9} {llama_info.total_params:,d}"
    )
    lines.append("")
    ...
    for arch_name, results in [
        ("GPT-2", result.gpt2_results),
        ("LLaMA", result.llama_results),
    ]:
        lines.append(f"\n{arch_name} Results:")
        lines.append(
            f"  {'Mode':<18} {'BER':<8} {'PPL':<10} {'KL Div':<10} {'Top5%':<8} {'Cat%':<8} {'Corr':<10} {'Det':<10}"
        )
        lines.append("  " + "-" * 90)
        ...
        for cache_mode, ber_results in results.items():
            for ber, trial in ber_results.items():
                ...
                ppl_str = (
                    f"{trial.perplexity:.2f}"
                    if trial.perplexity < 1e6
                    else f"{trial.perplexity:.1e}"
                )
                kl_str = (
                    f"{trial.kl_divergence:.4f}"
                    if hasattr(trial, "kl_divergence")
                    else "N/A"
                )
                top5_str = (
                    f"{trial.top5_accuracy*100:.1f}"
                    if hasattr(trial, "top5_accuracy")
                    else "N/A"
                )
                cat_str = (
                    f"{trial.catastrophic_rate*100:.1f}"
                    if hasattr(trial, "catastrophic_rate")
                    else "N/A"
                )
                corr_str = (
                    f"{trial.errors_corrected:,}" if trial.errors_corrected > 0 else "-"
                )
                det_str = (
                    f"{trial.errors_detected:,}"
                    if hasattr(trial, "errors_detected") and trial.errors_detected > 0
                    else "-"
                )
                lines.append(
                    f"  {cache_mode:<18} {ber_str:<8} {ppl_str:<10} {kl_str:<10} {top5_str:<8} {cat_str:<8} {corr_str:<10} {det_str:<10}"
                )
    ...
    lines.append(
        f"  1. Layer Detection: Both {gpt2_info.layer_type} (GPT-2) and {llama_info.layer_type} (LLaMA) correctly detected"
    )
    lines.append("  2. Error Correction: Active on both architectures")
    lines.append(
        "  3. PPL Stability: Both maintain stable PPL under Hamming protection"
    )
    lines.append("")
...
def _find_kv_projection_layers(model: nn.Module) -> List[Tuple[str, nn.Module]]:
    ...
    for name, module in model.named_modules():
        ...
        if module_type == "Conv1D" and "c_attn" in name_lower:
            ...
            for pattern in ["k_proj", "v_proj", "key_proj", "value_proj"]
        ):
            target_layers.append((name, module))
...
def plot_comparison(
    ...
    for cache_mode, ber_results in result.gpt2_results.items():
        ...
        ax1.plot(
            ...
            ppls,
            marker="o",
            color=colors.get(cache_mode, "gray"),
            label=label,
            linewidth=2,
        )
