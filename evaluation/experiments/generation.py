"""
Text Generation Demo for Qualitative Analysis.

Demonstrates the qualitative impact of bit errors on LLM text generation,
showing how different error-correcting codes preserve output quality.

This module provides lightweight infrastructure for generation comparisons,
used by the unified benchmark runner.

Uses GPU-native Triton kernels for ECC encode/decode and fault injection.
"""
...
# Map cache_mode strings to ECCShimConfig parameters
MODE_CONFIG = {
...
def run_generation_demo(
    ...
    """
    Run qualitative text generation comparison across protection strategies.

    Uses GPU-native Triton kernels for ECC operations and fault injection.

    Args:
        model: HuggingFace model (already loaded)
        tokenizer: HuggingFace tokenizer
        prompts: List of prompts to generate from
        cache_modes: List of cache modes to compare
        ber: Bit error rate (default: 0.05 for visible effects)
        seed: Random seed for reproducibility
        max_new_tokens: Number of tokens to generate
        device: Device model is on

    Returns:
        List of GenerationResult for each (prompt, mode) combination
    """
    ...
    for prompt in prompts:
        ...
        for cache_mode in cache_modes:
            # Reset seed for deterministic generation across modes
            ...
            config = ECCShimConfig(
                ...
                inject_errors=(ber > 0),
                seed=seed,
                num_blocks=512,  # Sufficient for generation
                block_size=16,
...
def analyze_generation(prompt: str, generated: str, cache_mode: str) -> str:
    """
    Analyze generated text to identify failure mode.

    Returns:
        Analysis tag describing the output quality
    """
    if cache_mode == "fp16":
        return "BASELINE"

    # Check for repetition loops (attention sink)
    words = generated.split()
    if len(words) > 5:
        # Check if any word repeats more than 3 times consecutively
...
def format_generation_results(results: List[GenerationResult]) -> str:
    ...
    lines.append("  - ATTENTION SINK: Repetition loop from bit flip causing softmax collapse")
    lines.append("  - SEC: Single-error correction (Hamming 7,4) - may miscorrect 2-bit errors")
    lines.append("  - SECDED: Single-error correct, double-detect (Hamming 8,4)")
    lines.append("  - SECDED+INTERP: SECDED with interpolation for detected double errors")
...
def results_to_dict(results: List[GenerationResult]) -> List[Dict[str, Any]]:
    """Convert GenerationResult list to list of dicts for JSON serialization."""
    return [
        {
...
# Default prompts for generation demos
...
# Default cache modes for generation (all 7 modes)
