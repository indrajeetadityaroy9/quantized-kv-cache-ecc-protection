...
class TestAdaptiveUEP:
    def test_adaptive_uep_basic(self):
        ...
        sink_k_cache = torch.zeros(
            num_blocks,
            num_layers,
            num_heads,
            golay_codewords_per_head,
            dtype=torch.int32,
            device=device,
        )
        sink_v_cache = torch.zeros(
            num_blocks,
            num_layers,
            num_heads,
            golay_codewords_per_head,
                        ...
                        if pad_len > 0:
                            k_vals = torch.cat(
                                [
                                    k_vals,
                                    torch.zeros(
                                        pad_len, dtype=k_vals.dtype, device=device
                                    ),
                                ]
                            )
                            v_vals = torch.cat(
                                [
                                    v_vals,
                                    torch.zeros(
                                        pad_len, dtype=v_vals.dtype, device=device
                                    ),
                                ]
                            )
    ...
    def test_adaptive_uep_no_sink(self):
        ...
        sink_k_cache = torch.zeros(
            1, num_layers, num_heads, 1, dtype=torch.int32, device=device
        )
        sink_v_cache = torch.zeros(
            1, num_layers, num_heads, 1, dtype=torch.int32, device=device
        )
        ...
        assert (
            max_diff < 1e-5
        ), f"Adaptive with sink_boundary=0 should match regular Hamming84, diff={max_diff}"
