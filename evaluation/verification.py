"""
Algebraic Verification of ECC Codec Correctness.

This module verifies the mathematical properties of the ECC codecs to ensure
they implement valid linear error-correcting codes. These are necessary conditions
for the codecs to provide the claimed error correction/detection capabilities.

Verification Criteria:

    1. Null Space Condition:
       For all codewords c generated by G: H·c^T ≡ 0 (mod 2)
       This ensures the parity-check matrix correctly identifies valid codewords.

    2. Subspace Orthogonality:
       G·H^T ≡ 0 (mod 2)
       The row space of G and row space of H must be orthogonal over GF(2).
       This is the defining property of linear codes.

    3. Basis Independence:
       rank_GF(2)(G) = k
       The generator matrix rows must be linearly independent to generate
       all 2^k valid codewords without redundancy.

    4. Error Amplification (Δd_H):
       For each error pattern, measure:
           Δd_H = d_H(original, decoded) - d_H(original, corrupted)

       Interpretation:
           Δd_H < 0: Successful correction (decoded closer to original)
           Δd_H = 0: Error detected, no change (safe failure)
           Δd_H > 0: Miscorrection (decoded further from original - dangerous!)

       SECDED codes should have Δd_H ≤ 0 for all single and double errors.

Data Structures:
    - NullSpaceResult: Syndrome zero rate across all codewords
    - OrthogonalityResult: G·H^T orthogonality check
    - RankResult: GF(2) rank of generator matrix
    - ErrorAmplificationResult: Correction/detection/miscorrection rates
    - VerificationReport: Complete verification summary

Usage:
    reports = run_all_verifications(device="cuda")
    for name, report in reports.items():
        print(format_verification_report(report))

    # Individual codec
    report = verify_hamming84()
    assert report.all_passed, "Hamming84 verification failed!"
"""
